...
public class PMSetGDDFrame extends JFrame {
...
// end of declarations   
     ...
     */
    public PMSetGDDFrame(Frame SGHF, OutPutFrame OPF, Parameters myPara, StringTokenizer st) {
//this constructor is for loading a scenario from a file
        ...
        try {
            ...
            this.getVarieties();
            this.setWindow((PMGrowthStages) myParameters.myStages);
    ...
    /**
     *  Sets the backReferance attribute of the PMSetGDDFrame object
     *
...
//DE added this method
    ...
    /**
     *  Sets the window attribute of the PMSetGDDFrame object
     *
     *@param  myGrowthStates  The new window value
     *@exception  Exception   Description of the Exception
     */
    public void setWindow(PMGrowthStages myGrowthStates) throws Exception {
...
//  *************************** we populate the window
        jTitleLabel.setText(myGrowthStates.myType + " Proso Millet Growth Stages");
...
//        set ROW11 Radio button
    ...
    /**
     *  Gets the varieties attribute of the PMSetGDDFrame object
     *
     *@return                  The varieties value
     *@exception  IOException  Description of the Exception
     */
    private PMGrowthStages getVarieties() throws IOException {
        //    **************  get varieties ******************
        File[] myVarFiles;
        File myFile;
        ...
        PMGrowthStages thisStage = null;

        myVarFiles = varFiles.getFiles();
        for (int i = 0; i < myVarFiles.length; i++) {
            //debe changed file extensions to .dat
            //if (myVarFiles[i].getName().equalsIgnoreCase("Proso Millet.txt")) {
            if (myVarFiles[i].getName().equalsIgnoreCase("Proso Millet.dat")) {
                myFile = myVarFiles[i];
                try {
                    input = new BufferedReader(new FileReader(myFile));
                } catch (Exception ex) {
                    System.out.println("Can't open proso millet file: " + ex);
                }
...
// *********** make sure that the Generic variety is set as default
        for (int i = 0; i < allVarieties.size(); i++) {
            thisStage = (PMGrowthStages) allVarieties.get(i);
    ...
    void jCBVarietyType_actionPerformed(ActionEvent e) {
        PMGrowthStages tmpStage;

        for (int i = 0; i < allVarieties.size(); i++) {
            tmpStage = (PMGrowthStages) allVarieties.get(i);
...
//        reset all the radio buttons to default
    ...
    /**
     *  Sets the parameters attribute of the PMSetGDDFrame object
     */
...
// ************** get selected values ***********************
    ...
    /**
     *  Gets the Acode attribute of the PMSetGDDFrame object
     *
    ...
    /**
     *  Sets the growthStages attribute of the PMSetGDDFrame object, used when
    ...
    /**
     *  Sets the radioButton attribute of the PMSetGDDFrame object
     *
