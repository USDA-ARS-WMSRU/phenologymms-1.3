...
public class SBSetGDDFrame extends JFrame {
...
// end of declarations   
     ...
     */
    public SBSetGDDFrame(Frame SGHF, OutPutFrame OPF, Parameters myPara, StringTokenizer st) {
//this constructor is for loading a scenario from a file
        ...
        try {
            ...
            this.getVarieties();
            this.setWindow((SBGrowthStages) myParameters.myStages);
            this.setParameters();
...
//        jSouthPanel.setMinimumSize(new Dimension(100, 100));
        ...
        // set the text fields and radio butons
        ...
        // add other GridBag Layout panels to jPanel
    ...
    /**
     *  Sets the backReferance attribute of the SBSetGDDFrame object
     *
    ...
    /**
     *  Sets the window attribute of the SBSetGDDFrame object
     *
     *@param  myGrowthStates  The new window value
     *@exception  Exception   Description of the Exception
     */
    public void setWindow(SBGrowthStages myGrowthStates) throws Exception {
...
//  *************************** we populate the window
        jTitleLabel.setText(myGrowthStates.myType + " Spring Barley Growth Stages");
...
//        set ROW11 Radio button
  ...
  // bcv added path find
    ...
    /**
     *  Gets the varieties attribute of the SBSetGDDFrame object
     *
     *@return                  The varieties value
     *@exception  IOException  Description of the Exception
     */
    private SBGrowthStages getVarieties() throws IOException {
        //    **************  get varieties ******************
        File[] myVarFiles;
        File myFile;
        ...
        SBGrowthStages thisStage = null;

        myVarFiles = varFiles.getFiles();
        for (int i = 0; i < myVarFiles.length; i++) {
            //debe changed file extensions to .dat
       //if (myVarFiles[i].getName().equalsIgnoreCase("Spring Barley.txt")) {
            if (myVarFiles[i].getName().equalsIgnoreCase("Spring Barley.dat")) {
                myFile = myVarFiles[i];
                try {
                    input = new BufferedReader(new FileReader(myFile));
                } catch (Exception ex) {
                    System.out.println("Can't open spring barley file: " + ex);
                }
...
// *********** make sure that the Generic variety is set as default
    ...
    void jCBVarietyType_actionPerformed(ActionEvent e) {
        SBGrowthStages tmpStage;

        for (int i = 0; i < allVarieties.size(); i++) {
            tmpStage = (SBGrowthStages) allVarieties.get(i);
...
//        reset all the radio buttons to default
    ...
    /**
     *  Sets the parameters attribute of the SBSetGDDFrame object
     */
...
//        pass parameters to output frame
    ...
    /**
     *  Gets the Acode attribute of the BSSetGDDFrame object
     *
    ...
    /**
     *  Sets the SBgrowthStages attribute of the SBSetGDDFrame object, used when
