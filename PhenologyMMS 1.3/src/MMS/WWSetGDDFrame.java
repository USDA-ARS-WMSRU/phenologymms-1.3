...
public class WWSetGDDFrame extends JFrame {
...
// end of declarations
     ...
     */
    public WWSetGDDFrame(Frame SGHF, OutPutFrame OPF, Parameters myPara, StringTokenizer st) {
//this constructor is for loading a scenario from a file
        ...
        try {
            ...
            this.getVarieties();
            this.setWindow((WWGrowthStages) myParameters.myStages);
    ...
    } // end of constructor for loading a scenario from a file
    ...
    private void jbInit() throws Exception {
        ...
        jGDDPanel.setSize(400, 30); // was (200, 30), then 400, 30
     ...
     *@exception  Exception   Description of the Exception
     */
    public void setWindow(WWGrowthStages myGrowthStates) throws Exception {
...
//  *************************** we populate the window
        jTitleLabel.setText(myGrowthStates.myType + " Winter Wheat Growth Stages");
...
//        set ROW11 Radio button
  ...
  // bcv added path find
    ...
    /**
     *  Gets the varieties attribute of the WWSetGDDFrame object
     *
     *@return                  The varieties value
     *@exception  IOException  Description of the Exception
     */
    private WWGrowthStages getVarieties() throws IOException {
        //    **************  get varieties ******************
        File[] myVarFiles;
        File myFile;
        ...
        //String path = "C:/PhenologyMMS/Interface/Working/NetBeans_projects";
        ...
        WWGrowthStages thisStage = null;

        myVarFiles = varFiles.getFiles();
        for (int i = 0; i < myVarFiles.length; i++) {
            //if (myVarFiles[i].getName().equalsIgnoreCase("Winter Wheat.txt")) {
            if (myVarFiles[i].getName().equalsIgnoreCase("Winter Wheat.dat")) {
                myFile = myVarFiles[i];
                try {
                    input = new BufferedReader(new FileReader(myFile));
                } catch (Exception ex) {
                    System.out.println("Can't open winter wheat file: " + ex);
                }
...
// *********** make sure that the Generic variety is set as default
        for (int i = 0; i < allVarieties.size(); i++) {
            thisStage = (WWGrowthStages) allVarieties.get(i);
    ...
    void jCBVarietyType_actionPerformed(ActionEvent e) {
        WWGrowthStages tmpStage;

        for (int i = 0; i < allVarieties.size(); i++) {
            tmpStage = (WWGrowthStages) allVarieties.get(i);
...
//        reset all the radio buttons to default
        ...
        for (int i = 0; i < allVarieties.size(); i++) {
            tmpStage = (WWGrowthStages) allVarieties.get(i);
            if (tmpStage.myType.equalsIgnoreCase(jCBVarietyType.
    ...
    /**
     *  Sets the parameters attribute of the WWSetGDDFrame object
     */
...
//        pass parameters to output frame
    ...
    /**
     *  Gets the Acode attribute of the WWSetGDDFrame object
     *
    ...
    /**
     *  Sets the growthStages attribute of the WWSetGDDFrame object, used when
    ...
    private void setRadioButton(Enumeration e1, String ActionCode) {
        ...
        while (e1.hasMoreElements()) {
            tmpButton = (JRadioButton) e1.nextElement();
            if (tmpButton.getActionCommand().trim()
                .equalsIgnoreCase(ActionCode)) {
                tmpButton.setSelected(true);
            }
        }
    } // end of method setRadioButton

} // end of class WWSetGDDFrame
